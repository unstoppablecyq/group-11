  ...
  <view class="container">
    <view>
      <view class="item" :title="item.title" :clickable="true" @click="clickItem(item)" v-for="(item,i) in list">
        <view class="title"><text>{{item.place_depart}}未审核订单{{item.place_arrive}}</text></view>
        <view class="tail">
       ...
       <view class="tail" style="justify-content: flex-end;flex-direction: row;">
          <text class="time" style="color: #0000ff;font-size: 13px;">查看审核{{item.createtime}}</text>
        </view>
      </view>
    </view>
	<view>
	  <view class="item" :title="item.title" :clickable="true" @click="clickItem(item)" v-for="(item,i) in list">
	    <view class="title"><text>{{item.place_depart}}未审核订单{{item.place_arrive}}</text></view>
	    <view class="tail">
	      <text class="teacher">下单时间:20221212{{item.sendTime}}</text>
		  <text class="time">订单审核状态:未审核{{item.price}}</text>
	      <text class="time">治疗时间:未诊断{{item.toTime}}</text>
	    </view>
	    <view class="tail" style="justify-content: flex-end;flex-direction: row;">
	...
	<view>
	  <view class="item" :title="item.title" :clickable="true" @click="clickItem(item)" v-for="(item,i) in list">
	    <view class="title"><text>{{item.place_depart}}已审核订单{{item.place_arrive}}</text></view>
	    <view class="tail">
	    ...
	    <view class="tail" style="justify-content: flex-end;flex-direction: row;">
	      <text class="time" style="color: green;font-size: 13px;">修改审核{{item.createtime}}</text>
	    </view>
	  </view>
	</view>
  </view>
